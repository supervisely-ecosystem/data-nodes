    ...
    for mask_image in masks:
        ...
        for cluster in clusters:
            ...
            if (
                width == 0
                or height == 0
                or width / height > max_size_ratio_check
                or height / width > max_size_ratio_check
            ):
                continue
...
class FindCracknetsLayer(Layer):
    ...
    layer_settings = {
        ...
        "properties": {
            "settings": {
                ...
                "properties": {
                    "crack_class": {"type": "string", "minLength": 1},
                    "cracknet_class": {"type": "string", "minLength": 1},
                    "mask_resolution": {"type": "integer", "minimum": 1},
                    "loop_devide_eps": {"type": "number", "minimum": 0.001},
                    "min_area_in_bbox_coef": {"type": "number", "minimum": 0.00001},
                    "min_poly_bbox_area_coef": {"type": "number", "minimum": 0.00001},
                    "enable_subclustering": {"type": "boolean"},
                    "min_points_in_cluster": {"type": "integer", "minimum": 1},
                },
            }
        },
    }
    ...
    def define_classes_mapping(self):
        self.cls_mapping[ClassConstants.NEW] = [
            {"title": self.settings["cracknet_class"], "shape": Polygon.geometry_name()}
        ]
    ...
    def process(self, data_el: Tuple[ImageDescriptor, Annotation]):
        ...
        simple_polygons = ExtractCrackNets4(
            image_size_wh=imsize_wh,
            in_list=crack_lines,
        ...
        new_obj_class = ObjClass(name=cracknet_class, geometry_type=Polygon)
        ann = ann.add_labels(
            [
                Label(
                    geometry=Polygon(exterior=[(p[0], p[1]) for p in simple_polygon]),
