...
_CONTEXT = None
_DEPRECATED_MULTICORE_MESSAGE = (
    "imgaug.multicore.BatchLoader and BackgroundAugmenter are disabled in "
    "this patched build because they relied on unsafe pickle-based "
    "inter-process communication. Use imgaug.multicore.Pool instead."
)
...
class BatchLoader(object):
    ...
    def __init__(self, load_batch_func, queue_size=50, nb_workers=1,
                 threaded=True):
        raise RuntimeError(_DEPRECATED_MULTICORE_MESSAGE)
    ...
    def __del__(self):
        if not hasattr(self, "join_signal"):
            return
        if not self.join_signal.is_set():
            self.join_signal.set()
...
class BackgroundAugmenter(object):
    ...
    **Deprecated**. Augment batches in the background processes.
    ...
    def terminate(self):
        ...
        if hasattr(self, "queue_result") and not self.queue_result._closed:
